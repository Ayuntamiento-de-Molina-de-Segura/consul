class CreateSharedExtensionsSchema < ActiveRecord::Migration[6.0]
  def up
    unless schema_exists?(extensions_schema)
      execute_or_log_create_schema_warning("CREATE SCHEMA #{quote_schema_name(extensions_schema)}")
    end
  ...
  private
    ...
    def execute_or_log_create_schema_warning(statement)
      ...
      execute_or_log_warning(statement, allowed: create_permission?)
    end

    def execute_or_log_extension_warning(statement)
      execute_or_log_warning(statement, allowed: superuser?)
    end

    def execute_or_log_grant_usage_warning(statement)
      execute_or_log_warning(statement, allowed: schema_exists?(extensions_schema))
    end

    def execute_or_log_warning(statement, allowed:)
      if allowed
        execute statement
      else
